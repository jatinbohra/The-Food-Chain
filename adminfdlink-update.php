...
$id = $_REQUEST['id'];
  $data =mysqli_query($conn, "update add_foodlink set img='$folder', linkname='$linkname', Description='$Description', cuisine='$cuisine' where id ='$id'");
 	if($data=true)
 	{
	...
	<div class="main_container">
 		<div class="item">
 			 <div class="container" style="text-align: center; font-size: 20px;color:#2e4ead; " >
		 	 	<h2><b>-: Update Links :-</b></h2>
			 </div>
			<?php
			include("db.php"); 
				$id = $_REQUEST['id'];
				$data=mysqli_query($conn,"select * from add_foodlink where id = '$id'");
				$value=mysqli_fetch_assoc($data);
			?>
			
		  	<form  method="post"  enctype="multipart/form-data">
		  	<label for="fname"> Food Cuisine name:</label>
			  	<input type="text"  name="cuisine" placeholder="Enter Cuisine..."  value="<?php echo $value['cuisine']; ?>" required><br>
		  	<label for="fname">Food Link:</label>
		  	<input type="text" name="linkname" placeholder="Enter Name..."  value="<?php echo $value['linkname']; ?>" required><br>
		  	<label for="fname">Description :</label><br>
		    <textarea id="subject" name="Description" class="form-control form-control-lg" placeholder="Write something.." value="<?php echo $value['Description'] ?>" style="height:150px"></textarea><br>
		    <label for="file">Image Path:</label>
		    <input type="file" class="form-control" id="name" placeholder=""  value="<?php echo $value['img']; ?>" name="img"><br>
		    <input type="submit"  name="update" value="📌update Recipes Link ">
		   </form>
