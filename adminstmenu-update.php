...
$id = $_REQUEST['id'];
  $data =mysqli_query($conn, "update add_statemenu set image='$folder', title='$title' where id ='$id'");
 	if($data=true)
 	{
 		echo "data update";
 	}
...
<!--Update form -->
	<div class="main_container">
 		<div class="item">
			...
			include("db.php"); 
				$id = $_REQUEST['id'];
				$data=mysqli_query($conn,"select * from add-statemenu where id = '$id'");
				$value=mysqli_fetch_assoc($data);
			?>
			
		  	<form method="post" enctype="multipart/form-data">
		  		<label for="fname">Slide Food Cuisine name:</label>
			  	<input type="text" id="fname" value="<?php echo $value['title']; ?>" name="title" placeholder="Enter Cuisine..."class="form-control" required><br>
			  	<label for="file">Slide Path:</label>
			    <input type="file" class="form-control" id="name" value="<?php echo $value['image']; ?>" placeholder="" name="image"><br>
